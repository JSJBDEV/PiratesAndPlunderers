...
public class StructureBuilderBlock extends Block {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if(!world.isClient && hand==Hand.MAIN_HAND)
        {
            ...
            for (int i = -16; i < 16; i++) {
                for (int j = 0; j < 32; j++) {
                    for (int k = -16; k < 16; k++) {
                        if(world.getBlockState(pos.add(i,j,k)).getBlock()!= Blockwake.SHIP_BUILDER_MARKER
                                && world.getBlockState(pos.add(i,j,k)).getBlock()!= Blockwake.STRUCTURE_BUILDER_BLOCK
            ...
            try {
                ...
                FileUtils.writeLines(new File("./config/Blockwake/buildings/"+ vv+".blocks"),complete);
    ...
    public void onSteppedOn(World world, BlockPos pos, Entity entity) {
        ...
        if(world.getBlockState(pos.add(-16,0,-16)).getBlock()== Blocks.AIR){
            world.setBlockState(pos.add(-16,0,-16), Blockwake.SHIP_BUILDER_MARKER.getDefaultState());
        }
        if(world.getBlockState(pos.add(16,0,-16)).getBlock()== Blocks.AIR){
            world.setBlockState(pos.add(16,0,-16), Blockwake.SHIP_BUILDER_MARKER.getDefaultState());
        }
        if(world.getBlockState(pos.add(16,32,-16)).getBlock()== Blocks.AIR){
            world.setBlockState(pos.add(16,32,-16), Blockwake.SHIP_BUILDER_MARKER.getDefaultState());
        }
        if(world.getBlockState(pos.add(16,32,16)).getBlock()== Blocks.AIR){
            world.setBlockState(pos.add(16,32,16), Blockwake.SHIP_BUILDER_MARKER.getDefaultState());
        }
