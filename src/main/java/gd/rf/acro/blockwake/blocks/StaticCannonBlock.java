...
public class StaticCannonBlock extends HorizontalFacingBlock {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if(hand==Hand.MAIN_HAND && player.getStackInHand(hand).getItem()== Blockwake.ASTROLABE_ITEM)
        {
            if(player.getStackInHand(hand).hasTag() && player.inventory.count(Blockwake.CANNON_SHOT_ITEM)>0)
            {
                ...
                for (ItemStack item : player.inventory.main) {
                    if (item.getItem() == Blockwake.CANNON_SHOT_ITEM) {
                        item.decrement(1);
                        break;
                    }
                }
