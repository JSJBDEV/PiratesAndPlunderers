...
public class PortTownFeature extends Feature<DefaultFeatureConfig> {
    ...
    public boolean generate(ServerWorldAccess serverWorldAccess, StructureAccessor accessor, ChunkGenerator generator, Random random, BlockPos pos, DefaultFeatureConfig config) {
        ...
        for (int i = 0; i < floors; i++) {
            ...
            if(state==4)
            {
                serverWorldAccess.setBlockState(seaOrigin.up(5+i*4),PiratesAndPlunderers.STATIC_PIRATE_SPAWNER.getDefaultState(),3);
                serverWorldAccess.getBlockTickScheduler().schedule(seaOrigin.up(5+i*4),PiratesAndPlunderers.STATIC_PIRATE_SPAWNER,1);
            }
        }
