...
import gd.rf.acro.pap.engagement.ShipHasNoEntitiesException
import gd.rf.acro.pap.engagement.ShipIsAlreadyEngagedException
...
object EngageStartCommand {
    fun register(dispatcher: CommandDispatcher<ServerCommandSource?>) {
        dispatcher.register(
                CommandManager.literal("pap").then(CommandManager.literal("engage").then(CommandManager.literal("teleport").requires { src ->
    ...
    private fun executeEngagementStart(source: ServerCommandSource, attacker: Entity, defender: Entity): Int {
        if ((attacker !is SailingShipEntity) or (defender !is SailingShipEntity)) {
            source.sendFeedback(TranslatableText("pap.commands.engagement.nonBoatEntityError"), true)
            return 1;
        }

        try {
            EngagementManager.startEngagementWithShipEntities(attacker as SailingShipEntity, defender as SailingShipEntity)
            source.sendFeedback(TranslatableText("pap.commands.engagement.engagementBeginSuccess"), true)
            return 0;
        } catch (e: ShipHasNoEntitiesException) {
            source.sendFeedback(TranslatableText("pap.commands.error.ShipHasNoEntitiesException"), true)
        } catch (e: ShipIsAlreadyEngagedException) {
            source.sendFeedback(TranslatableText("pap.commands.error.ShipIsAlreadyEngagedException"), true)
        } catch (e: Exception) {
            source.sendFeedback(TranslatableText("pap.commands.error.GenericException %s", e.message), true)
        }
        return 1;
    }
